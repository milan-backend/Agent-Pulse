...
# ============================================
def validate_feature_access(db, workspace_id, feature_name):
    workspace = (
...
# ============================================
def validate_mission_access(db, workspace_id):
    workspace = (
        db.query(Workspace).filter(Workspace.id == workspace_id).first()
    )
    ...
    require_feature(workspace, "missions")
...
def mission_overview(
    ...
    # FEATURE
    validate_mission_access(db, workspace_id)

    # TOTAL
    total = (
        db.query(DurableStep)
        .filter(DurableStep.workspace_id == workspace_id)
        .count()
    )
...
# ============================================
# MISSION LIST (UPDATED WITH ADVANCED FILTER SEARCH)
# ============================================
@router.get("/list")
def list_missions(
    workspace_id: str = Header(...),
    q: str = None,  # Optional search string query
    status: str = None,  # Optional filter status parameter
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    # MEMBERSHIP
    ...
    # CASE-INSENSITIVE TEXT & ID SEARCH
    if q:
        search_filter = f"%{q}%"
        conditions = [DurableStep.task_name.ilike(search_filter)]

        try:
            uuid.UUID(q)
            conditions.append(DurableStep.id == q)
        except ValueError:
            pass
    ...
    # EXECUTE FETCH
    steps = query.order_by(DurableStep.created_at.desc()).limit(50).all()

    return [
        {
            "mission_id": str(step.id),
            "original_mission_id": (
            ...
            "runtime_controlled": step.runtime_controlled,
        }
        for step in steps
    ]
...
def get_mission(
    ...
    step = (
        db.query(DurableStep)
        .filter(
            DurableStep.id == step_id, DurableStep.workspace_id == workspace_id
        )
        .first()
    )
    ...
    return {
        ...
        "paused_at": str(step.paused_at) if step.paused_at else None,
        "resumed_at": str(step.resumed_at) if step.resumed_at else None,
        "killed_at": str(step.killed_at) if step.killed_at else None,
        "output_preview": (
            str(step.output_data)[:120] if step.output_data else None
        ),
    }
...
def retry_mission(
    ...
    if step.status != "failed":
        return {"message": "Mission is not failed"}
    ...
    return {"message": "Mission retry scheduled", "mission_id": str(new_step.id)}
...
def kill_mission(
    ...
    return {"message": "Kill endpoint coming soon", "mission_id": step_id}
...
def resume_mission(
    ...
    return {"message": "Resume endpoint coming soon", "mission_id": step_id}