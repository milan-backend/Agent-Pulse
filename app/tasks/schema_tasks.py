...
def sync_database_schemas(self, workspace_id: str, agent_id: str): # 👈 ADDED agent_id
    """
    Background worker that connects to client DB, inspects tables,
    and indexes them into ChromaDB for a specific agent.
    """
    db = SessionLocal()
    try:
        # 👈 Filter by BOTH workspace and agent
        ...
        if not config:
            print(f"❌ [SCHEMA SYNC] Configuration missing for workspace: {workspace_id}, agent: {agent_id}")
            return False

        print(f"🚀 [SCHEMA SYNC] Syncing schemas for agent: {agent_id}")
